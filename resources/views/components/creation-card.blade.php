<div class="flex flex-col space-y-2">
    ...
    <div class="flex flex-row justify-between items-center">
        ...
        <div class="group flex justify-end flex-nowrap space-x-2 {{ $likesAndComments ?? '' }}">
            ...
            <span class="transition-colors group-hover:text-aurora2">{{ $creation['commentCount'] }}</span>

            <i class="bi bi-heart group-hover:text-aurora0"></i>
            <span class="transition-colors group-hover:text-aurora0">{{ count($creation['likes']) }}</span>
        </div>
    </div>
</div>
